 ...
 * ✅ Browser Puppeteer singleton (évite relaunch à chaque PDF -> gros gain)
 * ✅ Page par génération (newPage), fermeture page après pdf
 * ✅ Options PDF plus légères (preferCSSPageSize + scale)
 * ✅ AUCUN waitUntil networkidle0
 * ✅ Timeouts + logs
 */
...
const REMOTE_ASSET_TIMEOUT_MS = Number(process.env.REMOTE_ASSET_TIMEOUT_MS || 8000); // 8s
...
/**
 * Démarre (ou réutilise) un browser Puppeteer.
 * Important: évite de relancer Chromium pour chaque semestre => gros gain.
 */
...
/**
 * Ferme proprement le browser (si besoin).
 * (Optionnel: tu peux l’appeler au shutdown process si tu veux)
 */
...
// Optionnel : fermeture sur exit (ne casse rien si absent)
      ...
      const req = client.get(
        ...
        (res) => {
          ...
          res.on("end", () => {
            ...
            return resolve(bufferToDataUrl(buf, String(contentType).split(";")[0]));
          });
        }
      );
...
/**
 * Convertit une image (chemin local) en base64 data URL.
 * Supporte aussi URL distante (download + base64) pour éviter réseau côté Chromium.
 */
...
function buildHtml(payload) {
  // (inchangé) — ton code intégral ici
  // NOTE: je reprends exactement ton buildHtml tel quel
  const {
    logoUACBase64,
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  html, body { width: 210mm; font-family: 'Times New Roman', Times, serif; color: #111; font-size: 8.5pt; background: #ffc8c8; }
  .page { width: 210mm; min-height: 297mm; padding: 5mm 7mm 5mm 7mm; background: #ffc8c8; }
  /* ... (inchangé, ton CSS complet) ... */
</style>
</head>
<body>
<div class="page">
...
exports.generateDocument = async (demande, etudiant, notesData, reference, institution, qrData) => {
  ...
  try {
    ...
    await page.pdf({
      ...
      // ✅ rend plus léger/faster sans casser ton design
